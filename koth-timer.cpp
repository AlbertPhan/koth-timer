...
// Game Stuff
#define DEFAULT_WIN_TIME 120 // Time to hold to win round
...
#define PATTERN_SPEED 0.2f // 0.9 will be good
...
#define BLUE_TEAM_HUE 935
#define MAX_ACCEL_HUE 341 // GREEN (120.0/360 *6*255) 
#define RESTING_HUE 200 // YELLOW -GREEN (100/360*6*255)
#define START_DECEL_HUE 20 // RED ORANGE (10/360*6*255)
#define MIN_DECEL_HUE 0  // RED(0.0/360*6*255)
...
static void fill_bar(uint32_t bright_color, uint32_t dim_color,
    ...
    for (int i = 0; i < NUM_LEDS; ++i) {
        ...
        const int pattern_position = (i + phase) % PATTERN_PERIOD;
...
static void render_rope(uint32_t blue_color, uint32_t red_color,
    uint32_t neutral_color, uint32_t blue_time_ms, uint32_t red_time_ms,
    uint32_t win_time_ms, bool blue_capping, bool red_capping,
    bool blue_won, bool red_won, bool round_active, bool tie_game,
    uint32_t now_ms, int phase_index) {
    ...
    // is this needed? if blue is won then their bar will be full leardy
    // if (round_active) {
    //     blue_fill = (int) ((blue_time_ms * center) / win_time_ms);
    //     red_fill = (int) ((red_time_ms * center) / win_time_ms);
    // } else {
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        ...
        if (i < center) {
            const int distance_from_center = center - i;
            ...
            const int distance_from_center = i - center;
        ...
        if (flash_winner && !flash_on) {
            if (blue_won && i < center) { // when blue won and pixel is on blue side turn off
                pixel_color = 0u;
            } else if (red_won && i > center) {
                pixel_color = 0u;
            }
        }
...
int main() {
    ...
    const uint32_t bar_color = hsv_to_grb(MAX_ACCEL_HUE, 255, 180);
    const uint32_t blue_team_color = hsv_to_grb(BLUE_TEAM_HUE, 255, 255);   // Blue team color.
    const uint32_t red_team_color = hsv_to_grb(RED_TEAM_HUE, 255, 255);      // Red team color.
